...
The upload notices are shown once per page. The Knowledge explorer and the Library sidebar's two trees (`Skills/` and `Plugins/`) each render the upload banners, and over one shared piece of state a single drop into `Skills/` used to paint the same "the upload became a suggestion" notice above both Library trees. Each tree names itself and keeps its own banners now: two drops in flight at once each report in the tree that took them, where before the second replaced the first's news and the first to finish cleared the second's. Banners from a branch the user has left go with it rather than sitting over the new tree describing a drop into the old one.

A refused upload shows the whole refusal. The banner was one truncated line — "Couldn't add knowledge-base/K…" — with the server's reason hidden in a tooltip. It now names the file, gives the server's reason in full over as many lines as it needs, and ends with what to do about it: try another folder or ask its owner for a permission refusal, a smaller file for a size limit, try again otherwise. The advice is chosen from the HTTP status the server refused with, not from the wording of its message, so rephrasing a refusal — or changing the upload cap — cannot leave the banner quietly giving the wrong answer.
