...
An external link in agent-authored HTML or in a page's frontmatter opens in a new tab instead of doing nothing, and a `#section` link scrolls the page it sits in instead of navigating to the parent folder. A skill page's images now refresh when a teammate replaces one, even while you are reading from your own suggestion branch.
