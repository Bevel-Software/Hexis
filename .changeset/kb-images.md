...
A web or email link in a page's frontmatter opens in a new tab instead of doing nothing, and an agent's HTML can open one through `bevel.navigate`. A link whose scheme the app does not recognise stays inert, as before. A `#section` link scrolls the page it sits in instead of navigating to the parent folder. A skill page's images now refresh when a teammate replaces one, even while you are reading from your own suggestion branch.
