...
A person who is only restricted on a path (named by a `deny`, holding nothing) carries the same note on their row. That row is the one place a mistyped address would otherwise stay invisible: a `deny` on the wrong email restricts nobody and looks fine.
...
The note is only ever shown on evidence. A chip is labelled when the server said `hasAccount: false` about that exact address, or when it answered a lookup of it and did not name it — and the suggest answer now says outright (`accountsKnown`) whether it ruled on accounts, so an older build, an autocomplete outage and a query too short to return anybody all leave the address unjudged rather than accusing every free-typed email. The suggestion list sorts an exact email match to the front so the fifteen-person cap can never hide it, and a later answer that says an account is gone takes the earlier claim back. The users lookup behind the flag is best-effort: it is read after the mutation has already committed, so a lookup that fails omits the field instead of turning a saved grant into a 500, and the one warning that failure writes escapes its caller-supplied text with `printable`, like the rest of the backend's logs.
