...
Saving a key now probes the provider and shows the verdict, and a **Test connection** button re-runs it on demand. Three states replace the old two: **Connected** (a real call just succeeded, with the time it was checked on hover), **Key saved** / **Signed in** / **No key needed** (in place, nothing has tested it), and **Not working** (the provider rejected it, quoting what it said). `Connected` is now the only word that asserts a working connection, it appears only on the tool page that ran the probe, and it is never shown without evidence.

The verdict is deliberately **not stored**. A saved verdict is evidence with a date on it, and a credential can be revoked, rotated, or expire between the check and the render — so **Connected** means "a call succeeded moments ago" rather than "a call succeeded once". Everywhere a list is built from what is merely saved — the library cards, the Secrets and Connect pages, a skill's integrations — the badge says what was actually done (**Key saved** / **Signed in**) instead of claiming a working connection.
