...
| **WP0** | Backend: unified groups module — `GET /api/groups` (merged DTO §2.2), requests table + 3 request endpoints, `listAllSummaries()`, wiring + invalidation, migration `0001` | `04` §4.2 with §2.2/D2 applied; tests `04` §7 backend (adapt DTO assertions) + `02` §7 backend service cases that still apply (legacy-root merge, sorting, degradation) | — | yes |
