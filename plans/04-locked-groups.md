...
Migration: run `pnpm db:generate` in `packages/core-backend` → commits `migrations/0001_group_access_requests.sql` + updated `migrations/meta/` (journal entry idx 1). Rows are never deleted (audit trail, mirroring the revoked-not-deleted pattern).
...
| `packages/core-backend/migrations/0001_group_access_requests.sql` (+ `meta/` updates) | generated by `pnpm db:generate` after the schema edit |
...
**M2 — Request persistence + endpoints.** Schema + migration `0001`, `AccessRequestsService`, `POST /api/groups/:name/access-requests`, `GET /api/groups/access-requests`, `POST .../:id/dismiss`, real `hasRequested` + lazy fulfillment in `GET /api/groups`. *Verify:* POST twice → one pending row; GET as group admin lists it; grant read via the existing access UI → GET no longer lists it and the row is `fulfilled`; dismiss path flips to `dismissed`.
...
- [ ] Migration `0001_group_access_requests.sql` applies idempotently on boot after `0000`.
