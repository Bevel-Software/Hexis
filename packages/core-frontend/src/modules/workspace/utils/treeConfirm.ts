...
export function countFiles(entry: FileTreeEntry): number {
  if (entry.type === 'file') return 1;
  return (entry.children ?? []).reduce((n, child) => n + countFiles(child), 0);
}
...
export function deleteSentence(entry: FileTreeEntry): string {
  if (entry.type === 'file') return `Delete ${entry.name}?`;
  const n = countFiles(entry);
