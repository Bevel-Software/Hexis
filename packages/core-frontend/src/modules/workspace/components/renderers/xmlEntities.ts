...
/**
 * Every comment/CDATA/PI span in the part, in document order, as
 ...
 * element scans to the END OF THE PART and back for every paragraph of a deck
 * that contains no sections at all — which is nearly every real deck — and
 * that made ordinary rendering quadratic, on the user's own main thread. Here
 * each opener's `indexOf` resumes past the last span it found, so the scans
 * are disjoint and the whole index costs one pass; a part with no sections
 * leaves it empty and every later lookup is a no-op.
 */
 ...
 * have exhausted the tab before the deck ever rendered. PowerPoint emits no
 * XML comments at all, so no real deck comes near this.
 */
 ...
 *
 * Skipping sections only at the OPENER end was half a rule: `<a:p>a<!-- </a:p>
 * -->b</a:p>` still ended at the commented close tag, truncating a paragraph
...
export function xmlElementBlocks(xml: string, names: readonly string[]): XmlElementBlock[] {
  ...
  // one, and guessing either way misreads the part (see MAX_SECTIONS).
