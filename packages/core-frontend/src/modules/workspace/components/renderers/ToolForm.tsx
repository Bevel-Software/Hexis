...
const inputCls =
  'w-full rounded border border-slate-300 px-2 py-1 text-xs focus:border-slate-500 focus:outline-none disabled:bg-slate-50 disabled:text-slate-500';
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="flex items-center gap-2 text-[11px] text-slate-600">
        ...
        <span className="text-slate-400">(uncheck for local-only tools, e.g. a localhost MCP server)</span>
      </label>
  ...
  return (
    <div>
      <span className="mb-0.5 block text-[11px] font-medium text-slate-500">
        Access <span className="font-normal text-slate-400">(one role or `Name &lt;email&gt;` per line; prefix `deny ` to remove)</span>
      </span>
      <div className="grid grid-cols-2 gap-2">
        {ACCESS_VERBS.map((verb) => (
          <label key={verb} className="block">
            <span className="mb-0.5 block text-[10px] uppercase tracking-wide text-slate-400">{verb}</span>
            <textarea
              disabled={readOnly}
              rows={2}
              ...
              className="w-full resize-y rounded border border-slate-300 px-1.5 py-1 text-[11px] focus:border-slate-500 focus:outline-none disabled:bg-slate-50 disabled:text-slate-500"
  ...
  return (
    <div>
      ...
      <div className="space-y-1">
        {entries.length === 0 && readOnly && <p className="text-[11px] text-slate-400">None.</p>}
        {entries.map(([k, v], i) => (
...
function VariableRows({
  ...
  return (
    <div>
      <span className="mb-0.5 block text-[11px] font-medium text-slate-500">
        Variables <span className="font-normal text-slate-400">(who provisions each secret)</span>
      </span>
      <div className="space-y-1">
        {variables.length === 0 && readOnly && <p className="text-[11px] text-slate-400">None.</p>}
        {variables.map((v, i) => (
          <div key={i} className="flex gap-1">
            <input
              className={inputCls}
...
function InlineToolsEditor({
  ...
  return (
    <div>
      <span className="mb-0.5 block text-[11px] font-medium text-slate-500">Inline tools (advanced, raw JSON)</span>
      <textarea
        ...
        spellCheck={false}
        rows={8}
        className="w-full resize-y rounded border border-slate-300 bg-slate-50 p-2 font-mono text-[11px] focus:border-slate-500 focus:outline-none"
      />
