...
export const AGENT_CLIENTS: AgentClient[] = [
  ...
  },
  {
    ...
    // first.
    hint: 'For Claude Code, Claude Desktop, Cursor, Windsurf, Cline and any agent that runs on your machine: everything the hosted address gives, plus your plugins’ local-only tools. Needs Node 22.13+ or 24. The first time it starts, your browser opens so you can sign in. If the client reports that npx was not found, it was launched from the Dock or a desktop icon and cannot see your shell’s PATH — run `which npx` in a terminal (`where.exe npx` in PowerShell) and put the full path it prints in place of "npx" in the configuration below; if it then reports `env: node: No such file or directory`, that folder holds `node` too, so add it to the configuration’s "env" PATH — in front of the usual system folders, not instead of them, since the sign-in opens your browser through them.',
