 ...
 *
 * Same durability contract as `proposeChange`: `writeFile` on the branch
 * workspace auto-commits and pushes, so the proposal survives the tab closing
 * the moment this resolves. The existing-request lookup is by BRANCH against
 * the caller's own requests — the same reasoning as the skill page's `ownCr`:
