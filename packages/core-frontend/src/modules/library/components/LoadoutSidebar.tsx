...
export function LoadoutSidebar({
  ...
  if (total === 0) {
    return (
      <aside
        ...
        className="flex h-full w-14 shrink-0 flex-col items-center gap-3 border-r border-slate-200 bg-gradient-to-b from-white to-[#fafbfd] py-4 shadow-[6px_0_30px_rgba(22,35,58,0.06)] transition-[width] duration-300"
      >
        <div className="flex h-[26px] w-[26px] items-center justify-center rounded-full bg-[#f0f3f7] text-xs font-bold text-slate-400 shadow-[inset_0_0_0_1px_#e2e8f0]">
          0
        </div>
        <div className="text-[10.5px] font-bold uppercase tracking-[.2em] text-slate-400 [writing-mode:vertical-rl]">
          Loadout
        </div>
      </aside>
    );
  ...
  const renderRow = (row: LoadoutRow) => (
    ...
    >
      <div className="min-w-0">
        <div className="truncate text-[12.5px] font-semibold text-slate-800">{row.name}</div>
        <div className="text-[10.5px] text-slate-400">{row.status.text}</div>
      </div>
      ...
      <button
        ...
        className="lib-slot-kick absolute right-2 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-xs font-bold text-slate-400 hover:scale-110 hover:border-[#f3c4c4] hover:bg-[#fdecec] hover:text-[#c53030]"
        onClick={(e) => {
          e.stopPropagation();
          onRemove(row);
        }}
  ...
  return (
    <aside
      ...
      className="flex h-full w-72 shrink-0 flex-col border-r border-slate-200 bg-gradient-to-b from-white to-[#fafbfd] p-4 shadow-[6px_0_30px_rgba(22,35,58,0.06)] transition-[width] duration-300"
    >
      <div className="mb-3 flex items-baseline justify-between">
        <h2 className="whitespace-nowrap text-[15px] font-bold uppercase tracking-[.06em] text-[#0f766e]">
          Your loadout
        </h2>
        <div className={`text-[13px] font-bold text-slate-500 ${pulse ? 'lib-count-pulse' : ''}`}>
          {total}
        </div>
      </div>
      ...
      <div className="-mx-1.5 flex-1 overflow-y-auto px-1.5 pb-1">
        {both && (
          <div className="mx-1 mb-2 mt-1 text-[10px] font-bold uppercase tracking-[.14em] text-slate-400">
            Skills
          </div>
        )}
        {skills.map(renderRow)}
        {both && (
          <div className="mx-1 mb-2 mt-3 text-[10px] font-bold uppercase tracking-[.14em] text-slate-400">
            Integrations
          </div>
        )}
        {integrations.map(renderRow)}
        <div className="flex h-[46px] items-center justify-center rounded-[11px] border-[1.5px] border-dashed border-[#cbd7e4] text-[11.5px] tracking-[.05em] text-slate-400">
