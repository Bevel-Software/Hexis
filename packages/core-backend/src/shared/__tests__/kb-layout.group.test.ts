...
import { groupOfPath, GROUPS_DIR } from '@bevel-software/platform-shared';
...
describe('groupOfPath', () => {
  ...
  it('returns null for content directly under the root — it has no group folder', () => {
    // The regression this guards: `Groups/slack.tool` must NOT report a group
    // called "slack.tool", and `Groups/GTM` must not report itself.
    expect(groupOfPath(`${GROUPS_DIR}/slack.tool`)).toBeNull();
  ...
  it('returns null outside the group root', () => {
    ...
    expect(groupOfPath('Skills/GTM/heyreach-campaign')).toBeNull();
    expect(groupOfPath('Tools/NewsAgent/serper.tool')).toBeNull();
