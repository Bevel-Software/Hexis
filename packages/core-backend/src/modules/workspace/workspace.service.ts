...
import type { AuthUser, IWorkspaceService, WorkspaceInfo, FileTreeEntry } from '@bevel-software/shared';
import { assertValidRelativePath, validateFilename, DEFAULT_BRANCH } from '@bevel-software/shared';
...
export class WorkspaceService implements IWorkspaceService {
  ...
  private async cloneProcessMapForBranch(workspaceDir: string, branch: string): Promise<void> {
    ...
    try {
      ...
      // freshly checked out. Runs on whichever branch the user loaded (not just
      // protected ones), only on a fresh clone (this block). Isolated in its own
