 ...
 *
 * A free function, not a method: it reads ONE file it is handed the path of
 * and decides from the bytes. Nothing about it varies with the step's
 * collaborators.
 */
...
export class GroupsToPluginsStep implements OnServerStart {
  ...
  async run(ctx: ServerStartContext): Promise<StepResult> {
    ...
    for (const branch of await ctx.allBranches()) {
      await this.migrateBranch(branch, refusals);
    }
  ...
  private async migrateBranch(branch: KbBranch, refusals: string[]): Promise<void> {
    ...
    for (const entry of (await this.disk.listDir(rootOnDisk)) ?? []) {
      if (!entry.isDirectory() || isSkippedEntry(entry.name)) continue;
      const folderChanged = await this.migratePluginFolder(
        branch,
   ...
   */
  private async retireIgnoreRootRules(repoDir: string, branch: KbBranch, details: string[]): Promise<boolean> {
    let current: string;
    try {
   ...
   */
  private async migratePluginFolder(
    branch: KbBranch,
    folderDir: string,
    folderName: string,
    details: string[],
    ...
    if (!hasManifestEntry(entries)) {
      ...
      if (
        !(await this.layout.looksLikeLegacyPlugin(folderDir, entries)) ||
        (await this.layout.hasPluginBeneath(folderDir))
      ) {
        return false;
      }
    ...
    const convertOrMove = async (abs: string, rel: string, note: string): Promise<void> => {
      ...
      if (abs !== destDisk && !(await this.disk.exists(destDisk))) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && (await this.disk.exists(path.join(abs, 'SKILL.md')))) {
        if (!(await this.disk.exists(path.join(folderDir, PLUGIN_SKILLS_DIR, entry.name)))) {
    ...
    if (await this.disk.isDirectory(extToolsDir)) {
      for (const entry of (await this.disk.listDir(extToolsDir)) ?? []) {
    ...
    changed =
      (await this.foldIntoPluginFiles(
        branch,
        folderDir,
        relPlugin,
        folderName,
   ...
   */
  private async foldIntoPluginFiles(
    branch: KbBranch,
    folderDir: string,
    relPlugin: string,
    folderName: string,
    ...
    const mcp = (await this.disk.readJsonObject(path.join(folderDir, PLUGIN_MCP_FILE))) ?? {
