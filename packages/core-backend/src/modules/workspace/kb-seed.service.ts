...
function renderRolesYaml(adminEmails: readonly string[]): string {
  ...
  return (
    ...
    '#\n' +
    '# Generated by the Bevel platform at KB-seed time from SEED_ADMIN_EMAILS.\n' +
    'roles:\n' +
    '  Admin:\n' +
    `${lines}\n`
  );
      ...
      if (this.seedAdminEmails.length === 0) {
        throw new Error(
          `KB remote ${redact(this.kbRepoUrl)} is empty and cannot be seeded: ` +
            'set SEED_ADMIN_EMAILS to the initial Admin email(s). A seeded KB with ' +
            'no Admin is unusable (access resolution requires at least one Admin).',
        );
      }
    ...
    try {
      ...
      if (!(await this.exists(path.join(repoDir, 'roles.yaml')))) {
        if (this.seedAdminEmails.length > 0) {
          ...
          console.warn(
            `[kb-seed] Branch "${branch}" is missing roles.yaml but SEED_ADMIN_EMAILS ` +
              'is unset — leaving it absent. Access resolution will fail until an ' +
              'Admin roles.yaml exists; set SEED_ADMIN_EMAILS or add roles.yaml manually.',
          );
        }
      }
