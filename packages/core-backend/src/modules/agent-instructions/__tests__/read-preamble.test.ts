...
describe('readAgentPreamble', () => {
  it('reads the default branch copy at <kbDirName>/mcp-description.md, creating the workspace first', async () => {
    ...
    const ws = workspace();
    expect(await readAgentPreamble(ws, testKbContext({ kbDirName: KB }), disk)).toBe('Acme.\n');
  ...
  it("spells the path with the deployment's own KB dir name", async () => {
    ...
    expect(await readAgentPreamble(workspace(), testKbContext({ kbDirName: 'kb' }), disk)).toBe('Renamed root.');
    expect(await readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).toBeNull(); // the default-named root has none
  });

  it('a file that is not there is an absence, not a fault', async () => {
    expect(await readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).toBeNull();
  });
  ...
  it('refuses a symlink: the secret it points at is never read, and it is not an absence either', async () => {
    ...
    await expect(readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).rejects.toThrow(/symlink, not a regular file/);
  });
  ...
  it('refuses a repository folder reached through a link — the whole path must be its own, not only the file', async () => {
    ...
    await expect(readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).rejects.toThrow(/through a symlink/);
  });
  ...
  it('refuses a directory squatting the name', async () => {
    ...
    await expect(readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).rejects.toThrow(/directory, not a regular file/);
  });
  ...
  it('an lstat that fails for any reason but absence throws, never an empty preamble', async () => {
    ...
    try {
      await expect(readAgentPreamble(workspace(), testKbContext({ kbDirName: KB }), disk)).rejects.toMatchObject({ code: 'EACCES' });
    } finally {
      spy.mockRestore();
    }
  });
  ...
  it('a workspace that cannot be located throws too', async () => {
    ...
    await expect(readAgentPreamble(ws, testKbContext({ kbDirName: KB }), disk)).rejects.toBe(eio);
  });
});
