...
export class JoinRequestsService {
  ...
  /**
   * Open join requests for `group`, each carrying only its still-pending
   * proposals. Requests with none left are reconciled away (rejected, branch
   * deleted) and omitted.
   *
   * `actor` is who the reconciliation acts as — a manager of the group, since
  ...
  async list(
    ...
    for (const cr of crs) {
      if (cr.state !== 'open' || !isJoinBranchFor(cr.branch, group)) continue;
  ...
  async reconcile(
    ...
    if (cr.state !== 'open' || !isJoinBranchFor(cr.branch, group)) return false;
    await this.refresh();
    const accessPath = `${folder}/access.md`;
    const proposals = pendingProposals(
      await this.readAt(cr.branch, accessPath),
  ...
  private async settle(cr: ChangeRequest, actor: AuthUser): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `[groups] could not close settled join request #${cr.number}: ${
    ...
    try {
      ...
      console.warn(
        `[groups] closed join request #${cr.number} but could not delete "${cr.branch}": ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
    }
