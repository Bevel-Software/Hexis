...
describe('/api/plugins routes', () => {
  ...
  it('public demo: BOTH provisioning doors refuse, and the provisioner is never touched', async () => {
    ...
    server = h.server;
    for (const path of ['/api/plugins', '/api/plugins/personal']) {
      ...
      expect(body.error).toContain('github.com/Bevel-Software/Hexis');
    }
    expect(h.provision.createPlugin).not.toHaveBeenCalled();
  });
