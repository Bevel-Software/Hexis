...
**The toolset tracks the workspace.** A skill added, changed or removed on the workspace's default branch is in this server's very next `prompts/list` — prompts are read from the workspace on every request. A tool manual added, changed or removed reaches this server without a restart, when the connection is next used: whenever a tool call finishes or a client lists the tools, the server compares the workspace's catalog fingerprint with the one it registered — at most once every two seconds however busy the connection is, and never while it is idle, so an unused connection costs the workspace nothing. A listing runs its own check and waits for it, so the list you are handed is the current one; after a call, the change lands once that call finishes, so a new tool is callable from the call after that one — on a connection in use, within about five seconds of the commit. An idle connection catches up at its next use. When the catalog has moved, it re-registers the workspace's tools **and its local-only servers** (`local: true`, or a `type: "stdio"` command — those are fetched to your disk again and their child process is restarted by the re-registration), then sends the MCP tool-list-changed and prompt-list-changed notifications. A listing waits for that refresh, so the list it returns is the current one; a client that honours the notifications re-lists on its own, and one that caches the list it was given at connect time has to be asked to re-list, or reconnected. Two caveats: a refresh waits for a tool call that is still running (up to 15 seconds) before it re-registers, so a change committed mid-call lands after that call finishes; and a workspace older than this package cannot report its catalog, which the server says once on stderr before keeping the toolset it started with.
