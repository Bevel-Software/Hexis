...
**The toolset tracks the workspace.** A skill added, changed or removed on the workspace's default branch is in this server's very next `prompts/list` — prompts are read from the workspace on every request. A tool manual added, changed or removed reaches this server without a restart, within about five seconds of the commit, whether or not you were using the connection when it landed: the workspace announces the change down a stream this server keeps open (one parked request — nothing is asked on a timer, and an idle connection costs the workspace nothing), and the server re-registers the workspace's tools **and its local-only servers** (`local: true`, or a `type: "stdio"` command — those are fetched to your disk again and their child process is restarted by the re-registration), then sends the MCP tool-list-changed and prompt-list-changed notifications. It also compares the workspace's catalog fingerprint whenever the connection is used — a tool call finishing, a client listing the tools — at most once every two seconds however busy the connection is; that is what keeps the toolset current against a workspace that cannot announce anything, and it is why a listing is always the current list (a listing runs its own check and waits for it). A client that honours the notifications re-lists on its own, and one that caches the list it was given at connect time has to be asked to re-list, or reconnected. Three caveats: a refresh waits for a tool call that is still running (up to 15 seconds) before it re-registers, so a change committed mid-call lands after that call finishes; a workspace older than this package cannot report its catalog at all, which the server says once on stderr before keeping the toolset it started with; and the announcement stream is opened only against a workspace that advertises it (a `catalogEvents` capability on `/api/config`). A workspace that does not — an older release than this package, or one whose operator has turned it off — is not told apart from any other and nothing is said on stderr; those checks are simply the whole mechanism there, so a change lands at the connection's next use. A stream that WAS opened and then drops (a proxy that will not carry an event stream, a network that went away) does say so once on stderr, keeps reconnecting, and falls back to the same checks meanwhile.
