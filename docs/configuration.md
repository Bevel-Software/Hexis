...
| `DB_SCHEMA` | no | The Postgres schema this deployment's tables live in (default `public`). Set it to keep several knowledge bases in one database, each on a schema of its own |
